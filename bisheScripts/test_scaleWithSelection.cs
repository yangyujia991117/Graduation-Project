...
public class test_scaleWithSelection : MonoBehaviour
{
    ...
    public string singlePointDeltaPosition;//旋转点的位移
    public string rotationAxis;//旋转轴
    public string rotationAngle;//旋转的角度

    void Awake()
    {
        selectedObject = new GameObject("EmptyObject");
        singlePointDeltaPosition = "无";
        rotationAxis = "无";
        rotationAngle = "无";
    }
    ...
    private void OnGUI()
    {
        GUI.TextArea(new Rect(200,50,200,50),"旋转点的位移: " + singlePointDeltaPosition,400);
        GUI.TextArea(new Rect(420, 50, 200, 50), "旋转轴: " + rotationAxis,400);
        GUI.TextArea(new Rect(650, 50, 200, 50), "旋转的角度: " + rotationAngle,400);
    }
}
